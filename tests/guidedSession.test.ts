...
import { commitStep } from '../src/ui/guidedSession';
...
function commitArgs(
  ...
  return {
    ...
    draft: {
      phase: 'save' as const, method: 'tower', prereqs: [] as string[],
      settings: {}, result: {}
    },
    out: {
    ...
    notes: 'clear winner',
    photos,
    draftKey: `session:${project.id}:0:temperature`,
    label: 'Temperature'
  };
}
...
// The guided session is the second results-entry screen. The storage layer was
// fixed to reject when an IndexedDB transaction aborts at commit time; this
// screen has to honour that rejection the same way the classic wizard does, or
// a finished calibration measurement disappears with no message at all.
describe('guided session: saving a step result', () => {
  it('reports the failure instead of letting the user believe it was recorded', async () => {
    const project = await newProject();
    const restore = failNthPut(1);
    const saved = await commitStep(commitArgs(project));
    restore();
  ...
  it('leaves the on-screen project exactly as it was after a failed save', async () => {
    ...
    const restore = failNthPut(1);
    await commitStep(commitArgs(project));
    restore();
  ...
  it('records the result exactly once when the user retries after a failure', async () => {
    ...
    expect(await commitStep(commitArgs(project, photos))).toBe(false);
    restore();
    expect(await idb.getAll('photos')).toHaveLength(0);

    // The user presses "Save this result" again.
    expect(await commitStep(commitArgs(project, photos))).toBe(true);
  ...
  it('confirms the save on the happy path and keeps prior attempts in history', async () => {
    const project = await newProject();
    expect(await commitStep(commitArgs(project))).toBe(true);
    expect(await commitStep(commitArgs(project))).toBe(true);
