...
const STEP_SPECS: Record<CalibrationId, StepSpec> = {
  ...
  // Dual-nozzle idle-nozzle ooze control. Unlike every other step this one is
  // a guided checklist against the slicer and printer rather than a measured
  // test print, so it produces no profile value and needs nothing sliced. It is
  // also the only step outside DEFAULT_ORDER — projects targeting a bowden
  // auxiliary nozzle carry it, single-nozzle projects never do.
  'ooze-control': {
    produces: [],
    valueRanges: {},
    needsSlicing: false,
...
/**
 * Canonical sequence including optional steps that live OUTSIDE DEFAULT_ORDER.
 *
 * DEFAULT_ORDER is the plan every project gets; a step kept out of it is opt-in
 * (currently only `ooze-control`, carried by projects targeting a bowden
 * auxiliary nozzle). Those steps still need a position, so they sit immediately
 * before final-verification — where New Project inserts them — which also keeps
 * them after the pressure-advance and retraction results they depend on.
 */
export const CANONICAL_ORDER: CalibrationId[] = (() => {
  const optional = (Object.keys(CALIBRATIONS) as CalibrationId[])
    .filter((id) => !DEFAULT_ORDER.includes(id));
  if (!optional.length) return [...DEFAULT_ORDER];
  const out = [...DEFAULT_ORDER];
  const last = out.indexOf('final-verification');
  out.splice(last === -1 ? out.length : last, 0, ...optional);
  return out;
})();

/** The workflow registry: one dependency-aware definition per calibration step.
 *  Built from every defined calibration, not just DEFAULT_ORDER — the type is
 *  total over CalibrationId, so populating it from the default plan alone would
 *  leave an opt-in step `undefined` behind a non-optional type and throw the
 *  first time a project carrying it reached the workflow engine. */
export const WORKFLOW_STEPS: Record<CalibrationId, CalibrationStepDefinition> = CANONICAL_ORDER.reduce(
  (acc, id) => {
 ...
 *  sequence (a filtered `CANONICAL_ORDER`, so opt-in steps outside the default
 *  plan keep their place instead of being dropped from the workflow). */
